      ...
      children: [
        ...
        Expanded(
          child: LayoutBuilder(builder: (context, constraints) {
            // Decrease bar width, increase label width for more text visibility
            final barH = math.min(24.0,
                (constraints.maxHeight - 20) / math.max(data.length, 1) - 4);
            final labelMinWidth = 150; // Increased from 100 to show more text
                          ...
                          child: Padding(
                            ...
                            child: Row(
                              children: [
                                ...
                                Text(
                                  isMonetary ? fmtMoney(data[i]) : fmtValueWithCount(data[i]),
                                  style: TextStyle(
                                      fontSize: 12,
                                      fontWeight: FontWeight.w600,
                                      color: _wt.bodyText)),
...
;
    ...
    // Check if _meta contains total for percentage calculation
    final meta = model.binding['_meta'] as Map?;
    final hasTotal = meta?['total'] != null;
    ...
    // For users_by_type metric, show both count and percentage
    String fmtValueWithCount(double v) {
      final pct = (v / total * 100).toStringAsFixed(total >= 100 ? 0 : 1);
      final count = v.round().toString();
      if (isMonetary) {
        final money = fmtMoney(v);
        return '$money ($count - $pct%)';
      }
      if (_unit == '%') return '${v.toStringAsFixed(1)}%';
      return '$count - $pct%';
    }
                      ...
                      for (int i = 0; i < data.length && i < labels.length; i++)
                        Tooltip(
                          message: isMonetary
                              ...
                              : '${labels[i]}\n${fmtValueWithCount(data[i])}  ·  ${fmtPct(data[i])}',
                          waitDuration: const Duration(milliseconds: 150),
                          textStyle: TextStyle(
                              fontSize: 11,
                              color: _wt.bodyText,
